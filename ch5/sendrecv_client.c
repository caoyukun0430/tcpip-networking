...
int main(int argc, char* argv[])
{
	...
	for(i=0; i<3; i++)
	{
		...
		str_len=strlen(str_arr[i])+1; // adding \0
        printf("len %ld", strlen(str_arr[i]));
		write(sock, (char*)(&str_len), 4);
        // write(sock, read_buf, str_len); we can use this simply for echo
        write(sock, str_arr[i], str_len); // this is for server and client conversation, msg is different!
	}
	close(sock);
	return 0;
}
