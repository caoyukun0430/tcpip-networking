...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        // everything client send data, select returns the fd. Otherwise it returns 0 if nothing happens in timeout and continue
        if ((fd_num = select(fd_max + 1, &cpy_reads, 0, 0, &timeout)) == -1) {//开始监视,每次重新监听,
            // printf("break %d\n", fd_num);
            break;
        }
        if (fd_num == 0) {
            printf("nothing new from client %d\n", fd_num);
            continue;
        }
        printf("start for loop fd_max %d\n", fd_max);
